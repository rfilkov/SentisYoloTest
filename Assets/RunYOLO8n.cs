...
public class RunYOLO8n : MonoBehaviour
{
    ...
    void LoadModel()
    {
        ...
        modelLayers = model2.layers.Count;
    ...
    public void ExecuteML()
    {
        ...
        Graphics.Blit(sourceTex, targetRT, new Vector2(1f / aspect, 1), new Vector2(0, 0));
        displayImage.texture = targetRT;

        //// update the image aspect ratio
        //var imageSize = displayImage.rectTransform.sizeDelta;
        //imageSize.y = imageSize.x / aspect;
        //displayImage.rectTransform.sizeDelta = imageSize;

        long dtStart = System.DateTime.UtcNow.Ticks;
        var input = TextureConverter.ToTensor(targetRT, imageWidth, imageHeight, 3);
        ...
        Debug.Log($"Executed {modelLayers} layers in {(dtEnd - dtStart) * 0.0001f} ms. Results:\noutput - {output.shape}\nlabelIDs - {labelIDs.shape}");

        TensorFloat localOutput = output.ReadbackAndClone();
        TensorInt locallabelIDs = labelIDs.ReadbackAndClone();
        ...
        int boxesFound = localOutput.shape[0];
        //Draw the bounding boxes
        for (int n = 0; n < Mathf.Min(boxesFound, 200); n++)
        {
            var box = new BoundingBox
            {
                centerX = localOutput[n, 0] * scaleX - displayWidth / 2,
                centerY = localOutput[n, 1] * scaleY - displayHeight / 2,
                width = localOutput[n, 2] * scaleX,
                height = localOutput[n, 3] * scaleY,
                label = labels[locallabelIDs[n]],
            };
            DrawBox(box, n, displayHeight * 0.05f);
        }

        localOutput.Dispose();
        locallabelIDs.Dispose();
    }
